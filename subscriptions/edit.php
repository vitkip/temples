<?php
// filepath: c:\xampp\htdocs\temples\subscriptions\edit.php
ob_start();
session_start();
...
// ດຶງຂໍ້ມູນການສະໝັກສະມາຊິກທີ່ຕ້ອງການແກ້ໄຂ
try {
    ...
    // ກວດສອບສິດ admin ວ່າສາມາດແກ້ໄຂການສະໝັກສະມາຊິກນີ້ໄດ້ຫຼືບໍ່
    if ($is_admin && !$is_superadmin && $subscription['temple_id'] != $user_temple_id) {
        $_SESSION['error'] = "ທ່ານບໍ່ມີສິດແກ້ໄຂການສະໝັກສະມາຊິກນີ້";
        header('Location: ' . $base_url . 'subscriptions/');
        exit;
    }
...
$users_params = [];
if ($is_admin && $user_temple_id) {
    $users_sql .= " WHERE temple_id = ?";
    $users_params[] = $user_temple_id;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        ...
        header('Location: ' . $_SERVER['PHP_SELF'] . '?id=' . $subscription_id);
        exit;
    }
    ...
    if (empty($errors)) {
        try {
            $update_sql = "
                ...
                user_id = ?, temple_id = ?, plan_id = ?, status = ?, 
            ...
            $_SESSION['success'] = "ອັບເດດການສະໝັກສະມາຊິກສໍາເລັດແລ້ວ";
            header('Location: ' . $base_url . 'subscriptions/');
            exit;
...
$page_title = "ແກ້ໄຂການສະໝັກສະມາຊິກ #" . $subscription_id;
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex justify-between items-center mb-8">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">ແກ້ໄຂການສະໝັກສະມາຊິກ #<?= $subscription_id ?></h1>
            <p class="text-sm text-gray-600">ປັບປຸງຂໍ້ມູນການສະໝັກສະມາຊິກ</p>
        </div>
        <div>
    ...
    <!-- ແບບຟອມແກ້ໄຂການສະໝັກສະມາຊິກ -->
    <div class="bg-white rounded-lg shadow overflow-hidden">
        <form action="<?= $_SERVER['PHP_SELF'] ?>?id=<?= $subscription_id ?>" method="POST" class="p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <select name="temple_id" id="temple_id" class="form-select rounded-md shadow-sm w-full" <?= $is_admin && $user_temple_id ? 'disabled' : '' ?>>
                    ...
                    <?php if ($is_admin && $user_temple_id): ?>
            ...
            <div class="mb-6">
                ...
                <div class="flex space-x-4">
                    ...
                    <label class="inline-flex items-center">
                        <input type="radio" name="status" value="expired" class="form-radio" <?= $form_data['status'] === 'expired' ? 'checked' : '' ?>>
                        <span class="ml-2">ໝົດອາຍຸ</span>
                    </label>
                    <label class="inline-flex items-center">
                        <input type="radio" name="status" value="cancelled" class="form-radio" <?= $form_data['status'] === 'cancelled' ? 'checked' : '' ?>>
                        <span class="ml-2">ຍົກເລີກ</span>
                    </label>
                </div>
            </div>
