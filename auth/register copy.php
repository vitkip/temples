...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $form_data = [
        ...
        'role' => trim($_POST['role'] ?? 'user'), // ເພີ່ມການຮັບຄ່າ role
    ...
    if (empty($form_data['username'])) {
        ...
        if ($check_stmt->fetchColumn() > 0) {
            $errors[] = "ຊື່ຜູ້ໃຊ້ນີ້ມີຢູໃນລະບົບແລ້ວ";
        }
    }
    ...
    if (empty($errors)) {
        try {
            // ກຳນົດສະຖານະຕາມບົດບາດ
            $status = 'active'; // ຄ່າເລີ່ມຕົ້ນສໍາລັບ admin ແລະ user
            
            // ຖ້າເປັນ superadmin ຕ້ອງລໍຖ້າອະນຸມັດ
            ...
            $params = [
                ...
                $form_data['email'],
                $form_data['phone'],
                $form_data['role'],  // ໃຊ້ບົດບາດທີ່ຜູ່ໃຊ່ເລື່ອກ
                $form_data['temple_id'],
                $status  // ໃຊ້ສະຖານະຕາມເງື່ອນໄຂ
            ];
            ...
            // ປ່ອນສະຖານະສໍາເລັດສໍາລັດ
            if ($status === 'active') {
                $_SESSION['success'] = "ລົງທະບຽນສໍາເລັດແລ້ວ ທ່ານສາມາດເຂົ້າສູ່ລະບົບໄດ້ທັນທີ";
            } else {
            ...
            <form action="<?= $base_url ?>auth/register.php" method="POST" class="p-6 space-y-6" id="registerForm">
                ...
                <div>
                    ...
                    <div class="mt-1 input-group">
                        ...
                        function templeSelect() {
                            ...
                            highlight: 0,
                            selected: null,
                            temples: <?= json_encode($temples) ?>, // ຫຼືໃຊ້ @json($temples) ຖ້າເປັນ Blade
                            get filtered() {
                ...
                <div class="mt-6">
                    <label class="block text-sm font-medium text-gray-700">
                        ປະເພດຜູ້ໃຊ້ <span class="text-red-600">*</span>
                    </label>
                    
                    <div class="mt-4 flex flex-col sm:flex-row gap-4">
                        <div class="flex items-center">
                            <input type="radio" id="role_user" name="role" value="user" 
                                   class="h-4 w-4 text-amber-600 focus:ring-amber-500 cursor-pointer"
                                   <?= (!isset($form_data['role']) || $form_data['role'] === 'user') ? 'checked' : '' ?>>
                            <label for="role_user" class="ml-2 block text-sm text-gray-700 cursor-pointer">
                                <div class="font-medium">ຜູ້ໃຊ້ທົ່ວໄປ</div>
                                <div class="text-xs text-gray-500">ເຂົ້າເບິ່ງຂໍ້ມູນໄດ້ ແຕ່ບໍ່ສາມາດແກ້ໄຂຂໍ້ມູນໄດ້</div>
                            </label>
                        </div>
                        ...
                        <div class="flex items-center">
                            <input type="radio" id="role_admin" name="role" value="admin" 
                                   class="h-4 w-4 text-amber-600 focus:ring-amber-500 cursor-pointer"
                                   <?= (isset($form_data['role']) && $form_data['role'] === 'admin') ? 'checked' : '' ?>>
                            <label for="role_admin" class="ml-2 block text-sm text-gray-700 cursor-pointer">
                                <div class="font-medium">ຜູ້ດູແລວັດ</div>
                                <div class="text-xs text-gray-500">ສາມາດຈັດການຂໍ້ມູນວັດຂອງຕົນເອງໄດ້</div>
                            </label>
                        </div>
                        ...
                        <div class="flex items-center">
                            <input type="radio" id="role_superadmin" name="role" value="superadmin" 
                                   class="h-4 w-4 text-amber-600 focus:ring-amber-500 cursor-pointer"
                                   <?= (isset($form_data['role']) && $form_data['role'] === 'superadmin') ? 'checked' : '' ?>>
                            <label for="role_superadmin" class="ml-2 block text-sm text-gray-700 cursor-pointer">
                                <div class="font-medium">ຜູ້ດູແລລະບົບ</div>
                                <div class="text-xs text-gray-500">ສາມາດຈັດການຂໍ້ມູນທັງໝົດໃນລະບົບ (ຕ້ອງໄດ້ຮັບການອະນຸມັດ)</div>
                            </label>
                        </div>
                    </div>
                    
                    <div class="mt-2 text-xs text-amber-600">
                        <i class="fas fa-info-circle"></i> ສະເພາະຜູ້ສະໝັກເປັນຜູ້ດູແລລະບົບເທົ່ານັ້ນທີ່ຈະຕ້ອງລໍຖ້າການອະນຸມັດ
                </div>
